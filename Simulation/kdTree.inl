...
void KDTree<HullType>::construct(unsigned int node, AlignedBox3r const &box, unsigned int b, unsigned int n)
{
    ...
    auto c = static_cast<Real>(0.5) * (entity_position(m_lst[b + hal - 1])(max_dir) + entity_position(m_lst[b + hal])(max_dir));
    auto l_box = box;
    l_box.max()(max_dir) = c;
    auto r_box = box;
    r_box.min()(max_dir) = c;
...
template<typename HullType>
void KDTree<HullType>::traverse_depth_first(TraversalPredicate pred, TraversalCallback cb, TraversalPriorityLess const &pless) const
{
    if (m_nodes.empty())
        return;
...
template<typename HullType>
void KDTree<HullType>::traverse_depth_first(unsigned int node_index, unsigned int depth, TraversalPredicate pred, TraversalCallback cb,
...
template<typename HullType>
void KDTree<HullType>::traverse_breadth_first(TraversalPredicate const &pred, TraversalCallback const &cb, unsigned int start_node, TraversalPriorityLess const &pless,
...
void KDTree<HullType>::traverse_breadth_first_parallel(TraversalPredicate pred, TraversalCallback cb) const
{
    ...
    traverse_breadth_first([&start_nodes, &maxThreads, &targetDepth](unsigned int node_index, unsigned int depth)
                           {
...
{
    traverse_depth_first([&](unsigned int, unsigned int) { return true; }, [&](unsigned int node_index, unsigned int)
    {
