...
#pragma omp for schedule(static)
        for (int i = 0; i < (int) coPairs.size(); i++)
        {
            ...
            if (((co2->m_bodyType != CollisionDetection::CollisionObject::RigidBodyCollisionObjectType) &&
                 (co2->m_bodyType != CollisionDetection::CollisionObject::TetModelCollisionObjectType)) || !isDistanceFieldCollisionObject(co1) ||
                !isDistanceFieldCollisionObject(co2) || !AABB::intersection(co1->m_aabb, co2->m_aabb))
                continue;

            if ((co1->m_bodyType == CollisionDetection::CollisionObject::RigidBodyCollisionObjectType) &&
                (co2->m_bodyType == CollisionDetection::CollisionObject::RigidBodyCollisionObjectType) && ((DistanceFieldCollisionObject *) co1)->m_testMesh)
            {
                ...
                collisionDetectionRigidBodies(rb1, (DistanceFieldCollisionObject *) co1, rb2, (DistanceFieldCollisionObject *) co2, restitutionCoeff, frictionCoeff,
                                              contacts_mt);
                       ...
                       (co2->m_bodyType == CollisionDetection::CollisionObject::RigidBodyCollisionObjectType) && ((DistanceFieldCollisionObject *) co1)->m_testMesh)
            {
                ...
                collisionDetectionRBSolid(pd, offset, numVert, (DistanceFieldCollisionObject *) co1, rb2, (DistanceFieldCollisionObject *) co2, restitutionCoeff,
                                          frictionCoeff, contacts_mt);
                       ...
                       (co2->m_bodyType == CollisionDetection::CollisionObject::RigidBodyCollisionObjectType) && ((DistanceFieldCollisionObject *) co1)->m_testMesh)
            {
                ...
                collisionDetectionRBSolid(pd, offset, numVert, (DistanceFieldCollisionObject *) co1, rb2, (DistanceFieldCollisionObject *) co2, restitutionCoeff,
                       ...
                       (co2->m_bodyType == CollisionDetection::CollisionObject::TetModelCollisionObjectType) && ((DistanceFieldCollisionObject *) co1)->m_testMesh)
            {
                ...
                collisionDetectionSolidSolid(pd, offset, numVert, (DistanceFieldCollisionObject *) co1, tm2, (DistanceFieldCollisionObject *) co2, restitutionCoeff,
                                             frictionCoeff, contacts_mt);
            }
        }
    }
    ...
    for (unsigned int i = 0; i < contacts_mt.size(); i++)
    {
        for (unsigned int j = 0; j < contacts_mt[i].size(); j++)
        {
            if (contacts_mt[i][j].m_type == 1)
                addParticleRigidBodyContact(contacts_mt[i][j].m_index1, contacts_mt[i][j].m_index2, contacts_mt[i][j].m_cp1, contacts_mt[i][j].m_cp2,
                                            contacts_mt[i][j].m_normal, contacts_mt[i][j].m_dist, contacts_mt[i][j].m_restitution, contacts_mt[i][j].m_friction);
            else if (contacts_mt[i][j].m_type == 0)
                addRigidBodyContact(contacts_mt[i][j].m_index1, contacts_mt[i][j].m_index2, contacts_mt[i][j].m_cp1, contacts_mt[i][j].m_cp2, contacts_mt[i][j].m_normal,
                                    contacts_mt[i][j].m_dist, contacts_mt[i][j].m_restitution, contacts_mt[i][j].m_friction);
            else if (contacts_mt[i][j].m_type == 2)
            {
                addParticleSolidContact(contacts_mt[i][j].m_index1, contacts_mt[i][j].m_index2, contacts_mt[i][j].m_elementIndex2, contacts_mt[i][j].m_bary2,
                                        contacts_mt[i][j].m_cp1, contacts_mt[i][j].m_cp2, contacts_mt[i][j].m_normal, contacts_mt[i][j].m_dist,
                    ...
                    if (co2->collisionTest(X_l, 0.0, cp_l, n_l, dist))
                    {
                        ...
                        if (findRefTetAt(pd, tm2, co2, cp0, cp_tetIndex, cp_bary))
                        {
                            ...
                            contacts_mt[tid].push_back(
                                    {2, index, co2->m_bodyIndex, x_w, cp_w, n_w, dist, restitutionCoeff, frictionCoeff, tetIndex, cp_tetIndex, bary, cp_bary});
                        }
                    }
                }
            }
...
void DistanceFieldCollisionDetection::addCollisionBox(const unsigned int bodyIndex, const unsigned int bodyType, const Vector3r *vertices, const unsigned int numVertices,
...
void
DistanceFieldCollisionDetection::addCollisionSphere(const unsigned int bodyIndex, const unsigned int bodyType, const Vector3r *vertices, const unsigned int numVertices,
...
void
DistanceFieldCollisionDetection::addCollisionTorus(const unsigned int bodyIndex, const unsigned int bodyType, const Vector3r *vertices, const unsigned int numVertices,
...
void
DistanceFieldCollisionDetection::addCollisionCylinder(const unsigned int bodyIndex, const unsigned int bodyType, const Vector3r *vertices, const unsigned int numVertices,
...
void DistanceFieldCollisionDetection::addCollisionHollowSphere(const unsigned int bodyIndex, const unsigned int bodyType, const Vector3r *vertices,
                                                               const unsigned int numVertices, const Real radius, const Real thickness, const bool testMesh,
...
void DistanceFieldCollisionDetection::addCollisionHollowBox(const unsigned int bodyIndex, const unsigned int bodyType, const Vector3r *vertices,
                                                            const unsigned int numVertices, const Vector3r &box, const Real thickness, const bool testMesh,
                                                            const bool invertSDF)
{
...
void DistanceFieldCollisionDetection::addCollisionObjectWithoutGeometry(const unsigned int bodyIndex, const unsigned int bodyType, const Vector3r *vertices,
                                                                        const unsigned int numVertices, const bool testMesh)
{
...
bool DistanceFieldCollisionDetection::DistanceFieldCollisionHollowSphere::collisionTest(const Vector3r &x, const Real tolerance, Vector3r &cp, Vector3r &n, Real &dist,
                                                                                        const Real maxDist)
{
    const Vector3r d = x;
    const Real dl = d.norm();
...
void DistanceFieldCollisionDetection::DistanceFieldCollisionObject::initTetBVH(const Vector3r *vertices, const unsigned int numVertices, const unsigned int *indices,
                                                                               const unsigned int numTets, const Real tolerance)
{
...
bool DistanceFieldCollisionDetection::findRefTetAt(const ParticleData &pd, TetModel *tm, const DistanceFieldCollisionDetection::DistanceFieldCollisionObject *co,
                                                   const Vector3r &X, unsigned int &tetIndex, Vector3r &barycentricCoordinates)
{
