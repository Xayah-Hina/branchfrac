...
NeighborhoodSearchSpatialHashing::NeighborhoodSearchSpatialHashing(const unsigned int numParticles, const Real radius, const unsigned int maxNeighbors,
                                                                   const unsigned int maxParticlesPerCell) : m_gridMap(numParticles * 2)
{
...
void NeighborhoodSearchSpatialHashing::cleanup()
{
    ...
    for (unsigned int i = 0; i < m_gridMap.bucket_count(); i++)
    {
        ...
        if (kvMap)
        {
            for (Hashmap<NeighborhoodSearchCellPos *, NeighborhoodSearchSpatialHashing::HashEntry *>::KeyValueMap::iterator iter = kvMap->begin();
                 iter != kvMap->end(); iter++)
            {
