...
namespace PBD
{
    ...
    */
    ...
    public:
        ...
        static bool solve_GenericConstraint(const Real invMass[numberOfParticles],                            // inverse mass is zero if particle is static
                                            ...
                                            void(*constraintFct)(const unsigned int numParticles,
                                                                 ...
                                                                 void *userData, Eigen::Matrix<Real, dim, 1> &constraintValue),

                                            void(*gradientFct)(const unsigned int i,                            // compute dC / dxi
                                                               const unsigned int numParticles,
                                                               const Real invMass[],                            // inverse mass is zero if particle is static
                                                               const Vector3r x[],                        // positions of particles
                                                               void *userData, Eigen::Matrix<Real, dim, 3> &jacobian),
        ...
        */
        template<unsigned int numberOfParticles, unsigned int dim>
        static void approximateGradient(const unsigned int i,                            // compute dC / dxi
                                        const Real invMass[],                            // inverse mass is zero if particle is static
                                        const Vector3r x[],                        // positions of particles
                                        void *userData,
        ...
        static bool solve_GenericConstraint(const Real invMass[numberOfRigidBodies],                    // inverse mass is zero if body is static
                                            ...
                                            void(*gradientFct)(const unsigned int i,                                        // compute dC / dxi
                                                               ...
                                                               const Matrix3r inertiaInverseW[numberOfRigidBodies],        // inverse inertia tensor (world space) of bodies
                                                               const Quaternionr q[numberOfRigidBodies],                    // rotation of bodies
                                                               void *userData, Eigen::Matrix<Real, dim, 6> &jacobian),

                                            Vector3r corr_x[numberOfRigidBodies], Quaternionr corr_q[numberOfRigidBodies]);
        ...
        */
        ...
        static bool solve_GenericConstraint(const Real invMass[numberOfRigidBodies],                    // inverse mass is zero if body is static
                                            const Vector3r x[numberOfRigidBodies],                        // positions of bodies
                                            const Matrix3r inertiaInverseW[numberOfRigidBodies],        // inverse inertia tensor (world space) of bodies
                                            const Quaternionr q[numberOfRigidBodies], void *userData,

                                            void(*constraintFct)(const unsigned int numParticles,
                                                                 const Real invMass[numberOfRigidBodies],                    // inverse mass is zero if body is static
                                                                 const Vector3r x[numberOfRigidBodies],                        // positions of bodies
                                                                 const Matrix3r inertiaInverseW[numberOfRigidBodies],        // inverse inertia tensor (world space) of bodies
                                                                 const Quaternionr q[numberOfRigidBodies], void *userData, Eigen::Matrix<Real, dim, 1> &constraintValue),
    ...
    bool PositionBasedGenericConstraints::solve_GenericConstraint(
            ...
            void(*constraintFct)(const unsigned int numParticles, const Real invMass[],                            // inverse mass is zero if particle is static
                                 const Vector3r x[],                        // positions of particles
                                 void *userData, Eigen::Matrix<Real, dim, 1> &constraintValue),

            void(*gradientFct)(const unsigned int i,                            // compute dC / dxi
                               const unsigned int numParticles, const Real invMass[],                            // inverse mass is zero if particle is static
                               const Vector3r x[],                        // positions of particles
                               void *userData, Eigen::Matrix<Real, dim, 3> &jacobian),
    ...
    template<unsigned int numberOfParticles, unsigned int dim>
    void PositionBasedGenericConstraints::approximateGradient(const unsigned int i,                            // compute dC / dxi
                                                              const Real invMass[],                            // inverse mass is zero if particle is static
                                                              const Vector3r x[],                        // positions of particles
                                                              void *userData,
    ...
    template<unsigned int numberOfRigidBodies, unsigned int dim>
    bool PositionBasedGenericConstraints::solve_GenericConstraint(const Real invMass[numberOfRigidBodies],                    // inverse mass is zero if body is static
                                                                  const Vector3r x[numberOfRigidBodies],                        // positions of bodies
                                                                  const Matrix3r inertiaInverseW[numberOfRigidBodies],        // inverse inertia tensor (world space) of bodies
                                                                  const Quaternionr q[numberOfRigidBodies], void *userData,
