...
namespace PBD
{
    ...
    public:
        ...
        static void
        semiImplicitEulerRotation(const Real h, const Real mass, const Matrix3r &invInertiaW, Quaternionr &rotation, Vector3r &angularVelocity, const Vector3r &torque);
         ...
         */
        static void velocityUpdateFirstOrder(const Real h, const Real mass, const Vector3r &position,                // position after constraint projection	at time t+h
                                             const Vector3r &oldPosition,                // position before constraint projection at time t
                                             Vector3r &velocity);

        // -------------- angular velocity update (first order)  ------------------------------------------------
        static void
        angularVelocityUpdateFirstOrder(const Real h, const Real mass, const Quaternionr &rotation,                // rotation after constraint projection	at time t+h
        ...
        static void velocityUpdateSecondOrder(const Real h, const Real mass, const Vector3r &position,                // position after constraint projection	at time t+h
                                              const Vector3r &oldPosition,                // position before constraint projection at time t
        ...
        static void
        angularVelocityUpdateSecondOrder(const Real h, const Real mass, const Quaternionr &rotation,                // rotation after constraint projection	at time t+h
                                         const Quaternionr &oldRotation,            // rotation before constraint projection at time t
