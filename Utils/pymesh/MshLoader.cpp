...
void MshLoader::parse_nodes(std::ifstream& fin) {
    ...
    if (m_binary) {
        ...
        for (size_t i=0; i<num_nodes; i++) {
            ...
            vertices.emplace_back(m_nodes[node_idx*3], m_nodes[node_idx*3+1], m_nodes[node_idx*3+2]);
        }
...
void MshLoader::parse_elements(std::ifstream& fin) {
    ...
    if (!tet_elements.empty()) {
        // ------------------------------------------------------------
        tets.resize(tet_elements.size());
        for (int i = 0; i < tet_elements.size(); ++i)
        {
            tets[i] = tet_elements[i];
        }
        // ------------------------------------------------------------
