...
void DemoBase::init(int argc, char **argv, const char *demoName)
{
    ...
    m_exePath = PBD_EXE_DIR; // fixme
    m_dataPath = PBD_DATA_DIR; // fixme
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        else
        {
            ...
            if (FileSystem::isRelativePath(m_sceneFile))
                m_sceneFile = FileSystem::normalizePath(m_dataPath + "/scenes/" + m_sceneFile);
        }
    }
...
void DemoBase::renderTetModels()
{
    ...
    float surfaceColor[4] = {76.f / 255.f, 68.f / 255.f, 40.f / 255.f, 1};
