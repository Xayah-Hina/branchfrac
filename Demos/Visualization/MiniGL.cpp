...
void MiniGL::drawCube(const Vector3r &translation, const Matrix3r &rotation, float width, float height, float depth, float *color)
{
    ...
    Real val[16];
    val[0] = width * (rotation)(0, 0);
    val[1] = width * (rotation)(0, 1);
    val[2] = width * (rotation)(0, 2);
    val[3] = 0;
    val[4] = height * (rotation)(1, 0);
    val[5] = height * (rotation)(1, 1);
    val[6] = height * (rotation)(1, 2);
    val[7] = 0;
    val[8] = depth * (rotation)(2, 0);
    val[9] = depth * (rotation)(2, 1);
    val[10] = depth * (rotation)(2, 2);
    val[11] = 0;
    val[12] = (translation)[0];
    val[13] = (translation)[1];
    val[14] = (translation)[2];
    val[15] = 1;
...
void MiniGL::initTexture()
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, 3, IMAGE_COLS, IMAGE_ROWS, 0, GL_RGB, GL_UNSIGNED_BYTE, texData);  // Create texture from image data
...
void MiniGL::initTweakBarParameters()
{
    ...
    TwAddVarCB(m_tweakBar, "Rotation", TW_TYPE_QUAT4F, setRotationCB, getRotationCB, &m_quat, " label='Rotation' open help='Change the rotation.' ");

    // Add callback to toggle auto-rotate mode (callback functions are defined above).
    TwAddVarCB(m_tweakBar, "Wireframe", TW_TYPE_BOOL32, setWireframeCB, getWireframeCB, NULL, " label='Wireframe' key=w help='Toggle wireframe mode.' ");
...
{
    if ((m_context_major_version > major_version) || ((m_context_major_version == major_version) && (m_context_minor_version >= minor_version)))
        return true;
    return false;
}
