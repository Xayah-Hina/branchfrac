...
void createBodyModel()
{
    ...
    rb[rbIndex]->initBody(1.0, Vector3r(0.0, -0.5, 0.0), Quaternionr(1.0, 0.0, 0.0, 0.0), vdBox, meshBox, Vector3r(100.0, 1.0, 100.0));
    ...
    for (unsigned int i = 0; i < num_piles_z; i++)
    {
        ...
        {
            rb[rbIndex] = new RigidBody();
            rb[rbIndex]->initBody(100.0, Vector3r(current_x, 5.0, current_z), Quaternionr(1.0, 0.0, 0.0, 0.0), vdCylinder, meshCylinder, Vector3r(0.5, 10.0, 0.5));
            rb[rbIndex]->setMass(0.0);
    ...
    for (unsigned int i = 0; i < num_bodies_y; i++)
    {
        ...
        for (unsigned int j = 0; j < num_bodies_x; j++)
        {
            ...
            for (unsigned int k = 0; k < num_bodies_z; k++)
            {
                ...
                if (currentType == 0)
                {
                    rb[rbIndex]->initBody(100.0, Vector3r(current_x, current_y, current_z), q, //Quaternionr(1.0, 0.0, 0.0, 0.0),
