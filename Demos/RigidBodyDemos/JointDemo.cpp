...
void timeStep()
{
    ...
    for (unsigned int i = 0; i < numSteps; i++)
    {
        ...
        const Real currentTargetAngle =
                static_cast<Real>(M_PI * 0.5) - static_cast<Real>(M_PI * 0.5) * cos(static_cast<Real>(0.25) * TimeManager::getCurrent()->getTime());
...
void createBodyModel()
{
    ...
    for (unsigned int i = 0; i < 10; i++)
    {
        ...
        rb[3 * i]->initBody(0.0, Vector3r(startX, startY, 1.0), computeInertiaTensorBox(1.0, 0.5, 0.5, 0.5), Quaternionr(1.0, 0.0, 0.0, 0.0), vdStatic, meshStatic);

        // dynamic body
        rb[3 * i + 1] = new RigidBody();
        rb[3 * i + 1]->initBody(1.0, Vector3r(startX, startY - static_cast<Real>(0.25), static_cast<Real>(2.0)), computeInertiaTensorBox(1.0, width, height, depth),
                                Quaternionr(1.0, 0.0, 0.0, 0.0), vd, mesh);

        // dynamic body
        rb[3 * i + 2] = new RigidBody();
        rb[3 * i + 2]->initBody(1.0, Vector3r(startX, startY - static_cast<Real>(0.25), static_cast<Real>(4.0)), computeInertiaTensorBox(1.0, width, height, depth),
                                Quaternionr(1.0, 0.0, 0.0, 0.0), vd, mesh);
